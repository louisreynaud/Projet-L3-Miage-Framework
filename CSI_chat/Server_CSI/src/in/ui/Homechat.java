...
import csi.grp6.AccountManager;
import csi.grp6.Server;
import utils.Log;
import utils.StreamUtilities;
import utils.Task;
import utils.Log.ILogable;
...
public class Homechat extends JFrame implements ActionListener {
	...
	public Homechat() {
		...
		JLabel lblWorkingDirectory = new JLabel("Rerertoire de travail:");
	...
	private void changeInputState(boolean inputable) {
		btnStart.setText(inputable ? "Demarrer" : "Stop");
	...
	private void startServer() {
		if (!checkInput()) {
			JOptionPane.showMessageDialog(this, "Erreur input!");
		} else {
	...
	private static class ServerLogable implements Log.ILogable, Closeable {
		...
		public ServerLogable(JTextPane textPane, String dir) {
			...
			try {
				...
				appendLineInSafeThraed(String.format("Impossible de cree le fichier'%s'", e.getMessage()));
			}
		}
