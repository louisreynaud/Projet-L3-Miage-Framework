...
public class serverSocket implements Runnable{
	...
	public serverSocket(int port) throws IOException {
		sock = new ServerSocket(port);
      	System.out.println("Serveur est demarré  "+ InetAddress.getLocalHost().getHostAddress()+ " dans  port " + sock.getLocalPort());
	...
	public static void main(String args[]) throws IOException {
		...
		if(args.length == 1){
			...
			System.out.println("execute la commande suivante java P2P.serverSocket #port");
		}
	}
	...
	public void run() throws NullPointerException{
		...
		try {
			...
			System.out.println("Connextion etabli avec  client @ port "+ clientPort);
			...
			System.out.println("Peer ajoutez avec sucssé ");
	   ...
	   try { 
	       ...
	       {  
	  		...
	  		System.out.println("Connection avec client " + hostname +" @ " + clientPort + " closed");
	  	}
	}

	/*
	 * Method to remove  and Peers
	...
	private void addNewRFC(ObjectInputStream ois, ObjectOutputStream oos) throws ClassNotFoundException, IOException, NumberFormatException {
		...
		try {
				...
				rfcLists.add(rfcObj);
				System.out.println(" Ajoutez avec ");
				oos.writeObject(" Ajoutez avec ");
